...
void Dot::handle_input()
{
	...
	if (event.type == SDL_KEYDOWN)
	{
		...
		{
			std::cout << "left is true\n";
			left = true;
		}
		else if (event.key.keysym.sym == SDLK_RIGHT)
		{
			std::cout << "right is true\n";
			right = true;
		}
	}
	else if (event.type == SDL_KEYUP)
	{
		if (event.key.keysym.sym == SDLK_LEFT)
		{
			std::cout << "left is false\n";
			left = false;
		}
		if (event.key.keysym.sym == SDLK_RIGHT)
		{
			std::cout << "right is false\n";
			right = false;
		}
	}
}
...
void Dot::move()
{
	...
	{
		n = n - 3;
		x = ((LEVEL_WIDTH/2) + (CIRCLE_RADIUS) *cos(n*(PI/180)));
		y = ((LEVEL_HEIGHT/2) + (CIRCLE_RADIUS) *sin(n*(PI/180)));
	}
	if (right == true)
	{
		n = n + 3;
...
void Timer::pause()
{
	// if the timer is running and isn't alread paused
